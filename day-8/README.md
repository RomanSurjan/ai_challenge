...
`day-8` показывает, как история диалога влияет на размер следующего запроса, лимит контекста и примерную стоимость обращения к модели. Главный отчет теперь сводится к трем числам:

- `Input` - сколько токенов модель потратила на чтение входа;
- `Output` - сколько токенов модель потратила на генерацию ответа;
- `Overall` - сумма input + output.

Агент хранит историю в JSON и разделяет источники этих чисел:

- сырая локальная оценка до запроса;
- калиброванная локальная оценка до запроса;
- фактический `usage` DeepSeek API после ответа.
...
После успешного ответа в историю добавляется пара `user` + `assistant`. На следующем ходе эта пара снова попадет во входной prompt. Поэтому длинный диалог увеличивает input tokens и стоимость каждого следующего запроса.
...
Цены являются учебным примером. По умолчанию они равны `0`, чтобы не притворяться актуальным прайсом. Для демонстрации можно передать свои значения за 1M input/output tokens.
...
Token report:
  ...
  API usage:           input=... output=... overall=...
...
Web API `POST /api/chat` возвращает `token_report` вместе с ответом. Если DeepSeek вернул `usage`, API также отдаёт его отдельным полем `usage`. Страница показывает только три метрики под сообщением: `Input`, `Output`, `Overall`. При переполнении UI показывает понятную ошибку.
...
Ожидаемый результат: агент вернет ошибку про превышение лимита и не отправит запрос в DeepSeek.
...
- web API возвращает `token_report` и `usage`, если он пришел от API.

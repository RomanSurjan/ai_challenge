...
const (
	...
	defaultHistoryPath = "day-7/history.json"
)
...
func readConfig(args []string, stdin io.Reader) (config, error) {
	...
	flags := flag.NewFlagSet("day-7-agent", flag.ContinueOnError)
	...
	flags.StringVar(&cfg.HistoryPath, "history", envOrDefault("DAY7_HISTORY_PATH", defaultHistoryPath), "path to JSON conversation history")

...
Если пользователь передает только название фильма, это тоже считается запросом на рецензию. Если у названия есть несколько экранизаций, модель должна выбрать наиболее известную полнометражную версию и отметить неоднозначность в `short_description`.
...
Для рецензии `rating` должен быть непустой строкой с оценкой модели в формате `N/10`. `release_date` может быть датой, годом или пустой строкой, если дата неизвестна.
